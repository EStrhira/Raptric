...
const EBikeDetailSection = styled.section`
  padding: 80px 0;
  background: #000000;
  min-height: 100vh;
`
...
const EBikeContainer = styled.div`
  max-width: 1200px;
  margin: 0 auto;
  background: #1a1a1a;
  border-radius: 12px;
...
const EBikeImageSection = styled.div`
  img {
    width: 100%;
    height: auto;
    border-radius: 12px;
...
const GalleryTitle = styled.h3`
  ...
  &::after {
    ...
    flex: 1;
    height: 1px;
    background: linear-gradient(90deg, rgba(0, 166, 82, 0.3) 0%, transparent 100%);
  }
...
const ColorChip = styled.span<{ selected?: boolean }>`
  background: ${props => props.selected ? '#00a652' : 'rgba(255, 255, 255, 0.1)'};
  color: ${props => props.selected ? '#ffffff' : 'rgba(255, 255, 255, 0.9)'};
  padding: 0.5rem 1rem;
  border-radius: 20px;
  font-size: 0.9rem;
  border: 1px solid ${props => props.selected ? '#00a652' : 'rgba(255, 255, 255, 0.2)'};
  cursor: pointer;
  transition: all 0.3s ease;
...
const EBikeDetail: React.FC = () => {
  const { slug } = useParams<{ slug: string }>()
  const navigate = useNavigate()
  const [ebike, setEbike] = useState<EBike | null>(null)
  const [relatedEbikes, setRelatedEbikes] = useState<EBike[]>([])
  ...
  useEffect(() => {
    const fetchEBikeAndRelated = async () => {
      try {
        const [ebikeData, relatedData] = await Promise.all([
          (client as any).fetch(`*[_type == "ebike" && slug.current == $slug][0]{
            _id,
            name,
            slug,
            price,
          ...
          (client as any).fetch('*[_type == "ebike"] | order(featured desc, name asc) { _id, name, slug, price, discountPrice, shortDescription, image, images, colors, electricalSpecification, mechanicalSpecification, inStock, featured }[0...5]')
        ])
        
        setEbike(ebikeData)
        setRelatedEbikes(relatedData.filter((item: EBike) => item._id !== ebikeData?._id))
        setCurrentImageIndex(0)
      } catch (err) {
        setError('Failed to load eBike details')
        console.error('Error fetching eBike:', err)
      } finally {
        setLoading(false)
      }
    }

    fetchEBikeAndRelated()
  }, [slug])

  const goToPreviousImage = () => {
    if (ebike?.images && ebike.images.length > 0) {
      setCurrentImageIndex((prev) => (prev === 0 ? ebike.images!.length - 1 : prev - 1))
    }
  }

  const goToNextImage = () => {
    if (ebike?.images && ebike.images.length > 0) {
      setCurrentImageIndex((prev) => (prev === ebike.images!.length - 1 ? 0 : prev + 1))
    }
  }
  ...
  const getCurrentImage = () => {
    if (ebike?.images && ebike.images.length > 0) {
      return urlFor(ebike.images[currentImageIndex]).url()
    }
    return null
  }
  ...
  const handleAddToCart = () => {
    ...
    if (ebike.colors && ebike.colors.length > 0 && !selectedColor) {
    ...
    const cartItem = {
      ...
      name: ebike.name,
      category: 'eBike',
      price: ebike.discountPrice || ebike.price,
      image: ebike.images && ebike.images.length > 0 ? urlFor(ebike.images[0]).url() : ebike.image ? urlFor(ebike.image).url() : undefined,
  ...
  if (loading) {
    return (
      <EBikeDetailSection>
        <Container>
          <LoadingText>Loading eBike details...</LoadingText>
        </Container>
      </EBikeDetailSection>
    )
  }
  ...
  if (error || !ebike) {
    return (
      <EBikeDetailSection>
        <Container>
          <ErrorText>{error || 'eBike not found'}</ErrorText>
          <BackButton onClick={() => navigate('/ebikes')}>
  ...
  return (
    <EBikeDetailSection>
      ...
      <Container>
        ...
        <EBikeContainer>
          <EBikeHeader>
                ...
                {ebike?.images && ebike.images.length > 0 ? (
                  <>
                    <CarouselSlide isActive={true}>
                      <img 
                        src={getCurrentImage() || ''} 
                        alt={ebike.name || 'eBike image'} 
                      />
                    </CarouselSlide>
                    
                    {ebike.images.length > 1 && (
                      <>
                        ...
                        <ImageCounter>
                          {currentImageIndex + 1} / {ebike.images.length}
                        </ImageCounter>
                      </>
                    )}
                  </>
            ...
            <EBikeInfo>
              ...
              {ebike.colors && ebike.colors.length > 0 && (
                <ColorsSection>
                  <ColorsTitle>🎨 Available Colors</ColorsTitle>
                  <ColorsList>
          ...
          <SpecificationsSection>
            {ebike.electricalSpecification && (
              <SpecCard>
                <SpecTitle>⚡ Electrical Specifications</SpecTitle>
                <SpecContent>{ebike.electricalSpecification}</SpecContent>
              </SpecCard>
            )}
            {ebike.mechanicalSpecification && (
              <SpecCard>
                <SpecTitle>🔧 Mechanical Specifications</SpecTitle>
                <SpecContent>{ebike.mechanicalSpecification}</SpecContent>
              </SpecCard>
            )}
          </SpecificationsSection>
        </EBikeContainer>
      ...
      {relatedEbikes.length > 0 && (
        <Container>
          <RelatedSection>
            <RelatedTitle>🚴 Related eBikes</RelatedTitle>
            <RelatedGrid>
              {relatedEbikes.map((relatedEbike, index) => (
                <RelatedCard key={index}>
                  <RelatedImage>
                    {relatedEbike.images && relatedEbike.images.length > 0 ? (
                      <img src={urlFor(relatedEbike.images[0]).height(300).url()} alt={relatedEbike.name} />
                    ) : relatedEbike.image ? (
                      <img src={urlFor(relatedEbike.image).height(300).url()} alt={relatedEbike.name} />
                    ) : (
                      <i className="fas fa-bicycle"></i>
                    )}
                  </RelatedImage>
                  <RelatedName>{relatedEbike.name}</RelatedName>
                  <RelatedPrice>
