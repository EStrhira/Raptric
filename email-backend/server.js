...
const emailTemplates = {
  contactForm: (data) => ({
    ...
    html: `
      ...
      <body>
        <div class="header">
          <div class="logo">⚡ RAPTRIC Electric Bikes</div>
        ...
        <div class="footer">
          <p>This email was sent from the RAPTRIC Electric Bikes website</p>
          <p>${process.env.WEBSITE_URL || 'https://raptric.in'}</p>
        </div>
      </body>
      </html>
    `
  ...
  orderConfirmation: (data) => ({
    ...
    html: `
      ...
      <body>
        ...
        <div class="footer">
          <p>Thank you for choosing RAPTRIC Electric Bikes!</p>
          <p>${process.env.WEBSITE_URL || 'https://raptric.in'}</p>
        </div>
      </body>
      </html>
  ...
  welcomeEmail: (data) => ({
    subject: 'Welcome to RAPTRIC Electric Bikes!',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Welcome to RAPTRIC</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, #00a652, #008040); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
          .content { background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; }
      ...
      <body>
        <div class="header">
          <div class="logo">⚡ RAPTRIC Electric Bikes</div>
          <h1>Welcome to RAPTRIC!</h1>
          <p>Your journey into electric mobility starts here</p>
        </div>
        <div class="content">
          <h2>Hi ${data.userName},</h2>
          <p>Welcome to the RAPTRIC family! We're excited to have you join our community of electric bike enthusiasts.</p>
          ...
          <div style="text-align: center;">
            <a href="${process.env.WEBSITE_URL || 'https://raptric.in'}" class="cta-button">Start Shopping</a>
          </div>

          <p>If you have any questions, our support team is here to help you at every step of your electric bike journey.</p>
          
          <p>Happy riding!</p>
          <p><strong>The RAPTRIC Team</strong></p>
        </div>
        <div class="footer">
          <p>RAPTRIC Electric Bikes | Premium Electric Mobility Solutions</p>
          <p>You're receiving this email because you created an account on ${process.env.WEBSITE_URL || 'raptric.in'}</p>
        </div>
      </body>
      </html>
    `
...
async function sendEmail(to, subject, html) {
  try {
    const mailOptions = {
      from: `${process.env.FROM_NAME || 'RAPTRIC Electric Bikes'} <${process.env.FROM_EMAIL || 'noreply@raptric.in'}>`,
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`🚀 RAPTRIC Email Service running on port ${PORT}`);
